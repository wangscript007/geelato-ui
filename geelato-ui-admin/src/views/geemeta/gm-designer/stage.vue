<template>
  <div class="designer-stage-content">
    <div class="ui mini top attached tabular menu">
      <a class=" item active " data-tab="designer-tab-ui">界面</a>
    ...
    <div class="ui bottom attached tab segment stage-code" data-tab="designer-tab-code">
      <div id="mount-point">
      </div>
      <checkbox :opts="checkboxOpts"></checkbox>
    ...
    <div class="ui bottom attached tab segment stage-json" data-tab="designer-tab-json">
      codeMirror
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    watch: {
      'editingPage.id': function (val, oldVal) {
...
//      new Profile(options).$mount('#mount-point')
      this.$_initUi()
    },
    methods: {
      $_initUi: function () {
//        $(this.$el).find('.ui.dropdown').dropdown({action: 'hide'})
    ...
    components: {blankTemplate, checkbox}
  }
</script>
<style>
