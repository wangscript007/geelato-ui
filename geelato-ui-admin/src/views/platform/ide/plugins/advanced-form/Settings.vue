              ...
              <div class="content gl-hover-header">
                <div class="right floated gl-hover-content">
                  <i class="arrow up icon gl-action" title="向上"
                     @click="$utils.moveup(cards, index)" v-if="index!==0"></i>
                  <i class="arrow down icon gl-action" title="向下"
                     @click="$utils.movedown(cards, index)"
                  ...
                  <i class="remove red icon gl-action" title="删除" :class="$gl.ui.color.negative"
                     @click="$utils.remove(cards, index,'是否删除')"></i>
                </div>
                <div class="">{{dict[card.type]}}</div>
              </div>
              <template v-if="card.type==='tab'">
                <div class="content" v-if="card.items">
                  <div class="ui fluid action input" style="margin-bottom: 3px" v-for="(item,index) in card.items">
                    <label class="ui label">
                      <!--<i class="arrow up icon"  title="向上"-->
                      <!--@click="$utils.moveup(card.items, index)" v-if="index!==0"></i>-->
                      <i class="arrow down icon gl-action" title="向下"
                         @click="$utils.movedown(card.items, index)"
                         v-if="index!==card.items.length-1"></i>
                      <i class="remove red icon gl-action" title="删除" :class="$gl.ui.color.negative"
                         @click="$utils.remove(card.items, index,'是否删除')"></i>
                    </label>
...
//        this.form.description = this.editingPage.description
    ...
    methods: {
      ...
      $_loadMeta (card) {
        ...
        let thisVue = this
        this.$gl.ui.openVueByPath(this, '/views/platform/ide/plugins/advanced-form/SettingsForm',
          {
            title: '表单配置',
            opts: {
