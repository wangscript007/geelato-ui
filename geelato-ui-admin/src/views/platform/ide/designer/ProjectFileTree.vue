...
<script>
  ...
  import SimplePageDefinition from '../../../../pages/SimplePageDefinition'
  ...
  export default {
    ...
    watch: {
      'projectId': function (val, oval) {
        ...
        thisVue.$gl.data.query(entityNames.platform.common.treeNode, {treeId: project.id}, 'id,parent,text,type').then(function (res) {
          console.log('res>', res)
...
//                    alert('根节点不能删除！')
        ...
        $tree.bind('dblclick.jstree', function (event) {
          ...
          // 且叶子节点为文件类型才生效
          if (data.type) {
            thisVue.$_openPage(event, {node: data})
          }
        })
        $tree.jstree(true).open_all()

        function getFileNodeData ($node) {
          for (var type in thisVue.editorStore.fileTypes) {
