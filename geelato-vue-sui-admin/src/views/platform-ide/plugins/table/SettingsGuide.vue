<template>
  <div class="ui mini form" v-if="ui">
    <div class="ui five top attached steps">
      ...
      <div class="step" :class="{active:step==='stepB',disabled:!boundEntity}" @click="step='stepB'">
      ...
      <div class="step" :class="{active:step==='stepC',disabled:!boundEntity}" @click="step='stepC'">
      ...
      <div class="step" :class="{active:step==='stepD',disabled:!boundEntity}" @click="step='stepD'">
      ...
      <div class="step" :class="{active:step==='stepE',disabled:!boundEntity}" @click="step='stepE'">
        <i class="crosshairs icon"></i>
        <div class="content">
    ...
    <div class="ui attached segment" v-show="step==='stepA'">
      <div class="ui icon info message segment">
        ...
        <div class="">
          <template v-if="boundEntity">已绑定实体为<span style="font-weight: bold">{{boundEntity}}</span>
            <template v-if="selectedEntity&&selectedEntity!==boundEntity">，是否重新绑定为<span
      ...
      <meta-selector @select="selectMeta" :defaultEntity="selectedEntity"></meta-selector>
    </div>
    <!--查询条件-->
        ...
        <div class="eight wide column">
          ...
          <table class="ui compact  table">
            ...
            <tr v-for="(item,index) in selectedMeta">
              ...
              <!--<td>{{item.scale}}</td>-->
              <td><i class="plus icon" :class="$gl.ui.color.primary" style="cursor: pointer"
                     title="加入查询条件"
                     @click="addQueryMixItem(item,index)"></i></td>
            </tr>
          </table>
        </div>
        <div class="eight wide column">
          <div class="ui borderless secondary menu gl-header">
            ...
            <div class="ui right secondary mini menu">
              <div class="item toolbar-item">
                <!--<div class="ui mini button" :class="$gl.ui.color.primary"-->
                <!--@click="addQueryColumnItem({name: '无', title: '列', type: 'string', format: '',visible: true})">-->
                <!--添加虚拟列-->
                <!--</div>-->
              </div>
            </div>
            ...
            <tr v-for="(item,index) in ui.query.mix.fields">
              ...
              <td><i class="remove red icon" style="cursor: pointer" title="删除"
                ...
                <i class="arrow up icon" style="cursor: pointer" title="向上"
                   @click="upQueryMixItem(item,index)" v-if="index!==0"></i>
                <i class="arrow down icon" style="cursor: pointer" title="向下"
                   @click="downQueryMixItem(item,index)"
                   v-if="index!==ui.query.mix.fields.length-1"></i></td>
            </tr>
          </table>
        </div>
      </div>
          ...
          <table class="ui compact  table">
            ...
            <tr v-for="(item,index) in selectedMeta">
              ...
              <td><i class="plus icon" :class="$gl.ui.color.primary" style="cursor: pointer" title="加入"
                     @click="addQueryColumnItem(item,index)"></i></td>
            </tr>
          </table>
        </div>
        <div class="nine wide column">
          <div class="ui borderless secondary menu gl-header">
            ...
            <div class="ui right secondary mini menu">
              <div class="item toolbar-item">
                <div class="ui mini button" :class="$gl.ui.color.primary"
                     @click="addQueryColumnItem({name: '无', title: '列', type: 'string', el:'',format: '',visible: true})">
                  添加虚拟列
                </div>
              </div>
            </div>
          ...
          <table class="ui compact celled table">
            ...
            <template v-for="(item,index) in ui.table.columns">
              <tr :key="index">
                ...
                <td style="margin: 0;padding: 1px"><input type="text" v-model="item.title"/></td>
                <td style="margin: 0;padding: 1px"><input type="text" v-model="item.el"
                                                          placeholder="默认为当前字段值"/>
                </td>
                <td>
                  ...
                  <i class="remove red icon" style="cursor: pointer" title="删除"
                     @click="removeQueryColumnItem(item,index)"></i>
                  <i class="arrow up icon" style="cursor: pointer" title="向上"
                     @click="upQueryColumnItem(item,index)" v-if="index!==0"></i>
                  <i class="arrow down icon" style="cursor: pointer" title="向下"
                     @click="downQueryColumnItem(item,index)"
                     v-if="index!==ui.table.columns.length-1"></i></td>
              </tr>
                    ...
                    <tr>
                      ...
                      <td>
                        <div class="inline inline fields">
                          ...
                          <!--@click="item.textAlign=checkboxItem.value">-->
        ...
        <div class="five wide column">
          ...
          <table class="ui compact  table">
            ...
            <tr v-for="(item,index) in baseToolbarActions">
              ...
              <td>{{item.click}}</td>
              <td><i class="plus icon" :class="$gl.ui.color.primary" style="cursor: pointer" title="加入"
                     @click="addToolbarActionItem(item,index)"></i></td>
            </tr>
          </table>
        </div>
        ...
        <div slot="left">
          <table class="ui compact  table">
            ...
            <tr v-for="(item,index) in baseColumnActions">
              ...
              <td>{{item.click}}</td>
              <td><i class="plus icon" :class="$gl.ui.color.primary" style="cursor: pointer" title="加入"
                     @click="addColumnActionItem(item,index)"></i></td>
            </tr>
          </table>
        </div>
        <div slot="right">
  ...
  export default {
    ...
    mounted: function () {
      if (this.opts.editorStore.editingPage.content.opts.ui) {
        ...
        this.boundEntity = this.opts.editorStore.editingPage.content.opts.ui.entity
    ...
    methods: {
      ...
      bindEntity() {
        this.boundEntity = this.selectedEntity
        this.ui.entity = this.selectedEntity
        this.step = 'stepB'
      },
      addQueryMixItem(item) {
      ...
      removeQueryMixItem(item, index) {
        this.ui.query.mix.fields.splice(index, 1)
      },
      upQueryMixItem(item, index) {
      ...
      // removeToolbarActionItem (item, index) {
      //   this.ui.toolbar.dropdown.actions.splice(index, 1)
      // },
      // upToolbarActionItem (item, index) {
      ...
      // },
      // downToolbarActionItem (item, index) {
