...
<template>
    ...
    <table class="ui small compact form gl-form gl-col-24">
        <thead></thead>
        <tbody>
        <tr v-for="(row,index) in options.ui.layout" :key="index">
            <template v-for="(cell,cellIndex) in row" v-if="item=metaMap[Object.keys(cell)[0]]">
                <td :colspan="Object.values(cell)[0][0]" :key="cellIndex">{{item.title}}</td>
                <td :colspan="Object.values(cell)[0][1]" :key="cellIndex">
