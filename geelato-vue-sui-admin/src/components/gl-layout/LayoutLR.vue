  ...
  <div>
    ...
    <div :style="header">
      <page-header :mode="mode" :size="sidebar.size" @changeLayoutMode="changeLayoutMode"
                   ...
                   @changeColor="changeColor"></page-header>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      backgroundColor() {
        return this.$gl.ui.colorHex[this.$store.state.profile.userConfig['layout_color'].value]
      }
    },
    watch: {
      'backgroundColor': function (val, oval) {
        console.log('gl-layout > LayoutLR > backgroundColor: ', this.backgroundColor)
        this.$set(this.sidebar, 'background-color', this.backgroundColor)
      }
    },
    mounted() {
      let thisVue = this
